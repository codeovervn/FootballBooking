                        ...
                        <c:forEach var="contact" items="${listContact}" varStatus="status2">
                            ...
                            <tr>
                                ...
                                <c:if test="${contact.status != 2}">
                                    <td><a href="datve?id=${contact.id}&vong=${contact.vong}&matran=${contact.matran}" style="font-size: 13px; background-color: yellow">Đặt Vé</a></td>
                                    <td><a href="chitiettrandau?id=${contact.id}&vong=${contact.vong}&matran=${contact.matran}">Xem thông tin trận đấu</a></td>
                                    <c:if test="${contact.status == 0}">
                                        <!-- <td><img class="image" style="width: 27px!important;" src="http://livescore.24h.com.vn/images//arow-icon-2.gif"></td> -->
                                        <td><a href="chitiettrandau?id=${contact.id}&vong=${contact.vong}&matran=${contact.matran}"><img class="image" style="width: 27px!important;" src="resources/images/chuathidau.gif" alt="Chưa thi đấu"></a></td>
                                    </c:if>
                                    <c:if test="${contact.status == 1}">
                                        <td><a href="chitiettrandau?id=${contact.id}&vong=${contact.vong}&matran=${contact.matran}"><img src="resources/images/dangthidau.gif" width="15" height="17" alt="Đang thi đấu"></a></td>
                                    </c:if>
                                </c:if>
                                <c:if test="${contact.status == 2}">
                                    <td>Hết giờ</td>
                                    <td><a href="xemlaitrandau?id=${contact.id}&vong=${contact.vong}&matran=${contact.matran}">Xem lại</a></td>
                                    <!-- <td><img src="resources/images/kothidau.jpg" width="15" height="17" alt="Kết thúc"></td> -->
                                    <td>Đã kết thúc</td>
                                </c:if>
                            </tr>
                            </c:if>
